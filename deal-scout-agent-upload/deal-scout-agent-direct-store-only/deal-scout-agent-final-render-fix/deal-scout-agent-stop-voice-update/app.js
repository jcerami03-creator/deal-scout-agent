...
let shouldSearchAfterStop = false;
...
function startVoiceSearch() {
  ...
  if (isVoiceListening) {
    shouldSearchAfterStop = true;
    voiceSearchButton.textContent = "Searching...";
    voiceStatus.textContent = "Searching from what I heard...";
    recognition.stop();
    return;
  }
  ...
  if (!recognition) {
    ...
    recognition.addEventListener("result", (event) => {
      ...
      for (let index = event.resultIndex; index < event.results.length; index += 1) {
        ...
        if (event.results[index].isFinal) {
          voiceTranscript = `${voiceTranscript} ${transcript}`.trim();
        } else {
          interimTranscript = `${interimTranscript} ${transcript}`.trim();
        }
      }

      interimVoiceTranscript = interimTranscript;
      const preview = `${voiceTranscript} ${interimTranscript}`.trim();
    ...
    recognition.addEventListener("end", () => {
      const transcript = `${voiceTranscript} ${interimVoiceTranscript}`.trim();
      const shouldSearch = shouldSearchAfterStop || transcript.length > 0;

      isVoiceListening = false;
      shouldSearchAfterStop = false;
      ...
      if (shouldSearch && transcript) {
        ...
        voiceStatus.textContent = "I did not hear anything. Try again.";
      }
    });
  ...
  voiceSearchButton.textContent = "Stop & Search";
  voiceStatus.textContent = "Listening now. Say what you want, then click Stop & Search.";
  recognition.start();
}

function enforceSimpleVoiceLayout() {
  if (document.querySelector("#simpleVoiceLayout")) {
    return;
  }

  const style = document.createElement("style");
  style.id = "simpleVoiceLayout";
  style.textContent = `
    .searchForm {
